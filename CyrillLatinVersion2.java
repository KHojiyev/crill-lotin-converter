...
public class CyrillLatinVersion2 {
    public static String[] latin = {"a", "b", "d", "e", "e", "f", "g", "g‘", "g`", "g'", "h", "i", "j", "k", "q", "l", "m", "n", "o", "o'", "o‘", "o'", "p", "r", "s", "sh", "t", "u", "v", "x", "y", "z", "’", "A", "B", "Ch", "D", "E", "E", "F", "G", "G", "G`", "G'", "H", "I", "J", "K", "Q", "L", "M", "N", "O", "O‘", "O'", "", "P", "R", "S", "Sh", "SH", "T", "U", "V", "X", "Y", "Z", "’", "“", "”", "’", "ye", "Ye", "YE", "ya", "Ya", "YA", "yo", "Yo", "YO", "yu", "Yu", "YU", "ch", "Ch", "CH"};
    public static String[] cyrill = {"а", "б", "д", "э", "е", "ф", "г", "ғ", "ғ", "ғ", "ҳ", "и", "ж", "к", "қ", "л", "м", "н", "о", "ў", "ў", "ў", "п", "р", "с", "ш", "т", "у", "в", "х", "й", "з", "ъ", "А", "Б", "Ч", "Д", "Е", "Э", "Ф", "Г", "Ғ", "Ғ", "Ғ", "Ҳ", "И", "Ж", "К", "Қ", "Л", "М", "Н", "О", "Ў", "Ў", "", "П", "Р", "С", "Ш", "Ш", "Т", "У", "В", "Х", "Й", "З", "Ъ", "«", "»", "ь", "е", "Е", "Е", "я", "Я", "Я", "ё", "Ё", "Ё", "ю", "Ю", "Ю", "ч", "Ч", "Ч"};
    ...
    public static void main(String[] args) {
        System.out.println("==Welcome to CYRILL<=>LATIN converter==");
        while (true) {
            Scanner scanner = new Scanner(System.in);
            System.out.println("1.By file path\n" +
                    "2.By direct\n" +
                    "3.Enter text\n" +
                    "4.Exit");
    ...
    private static void enterText() {
        ...
        System.out.println("1.cyrill-latin");
        System.out.println("2.latin-cyrill");
        int choice = scanner.nextInt();
        switch (choice) {
            case 1:
                if (enterText != null) {
                    for (int i = 0; i < cyrill_words_upper.length; i++) {
                        enterText = enterText.replaceAll(cyrill_words_upper[i], latin_words_upper[i]);
                    }
                    for (int i = 0; i < cyrill_words_lower.length; i++) {
                        enterText = enterText.replaceAll(cyrill_words_lower[i], latin_words_lower[i]);
                    }
                    for (int i = cyrill.length - 1; i >= 0; i--) {
                        enterText = enterText.replaceAll(cyrill[i], latin[i]);
                    }
            ...
            case 2:
                if (enterText != null) {
                    for (int i = 0; i < latin_words_upper.length; i++) {
                        enterText = enterText.replaceAll(latin_words_upper[i], cyrill_words_upper[i]);
                    }
                    for (int i = 0; i < latin_words_lower.length; i++) {
                        enterText = enterText.replaceAll(latin_words_lower[i], cyrill_words_lower[i]);
                    }
                    for (int i = latin.length - 1; i >= 0; i--) {
                        enterText = enterText.replaceAll(latin[i], cyrill[i]);
                    }
                    System.out.println("Result:");
                    System.out.println(enterText + "\n");
                }
    ...
    private static void byDirect() {
        System.out.println("Select:");
        Scanner scanner = new Scanner(System.in);
        String dickPath = scanner.nextLine();


    }

    private static void byFilePath() {
        Scanner scanner = new Scanner(System.in);
        ...
        try (FileReader fileReader = new FileReader(filePath)) {
            ...
            switch (choice) {
                case 1:
                    cyrillLatin(bufferedReader);
                    break;
                case 2:
                    latinCyrill(bufferedReader);
                    break;
                default:
                    break;
            }
    ...
    private static void cyrillLatin(BufferedReader bufferedReader) throws IOException {
        String oneLine = " ";
        while ((oneLine = bufferedReader.readLine()) != null) {
            ...
            for (int length = cyrill.length - 1; length >= 0; length--) {
                oneLine = oneLine.replaceAll(cyrill[length], latin[length]);
            }
            list.add(oneLine);
    ...
    private static void latinCyrill(BufferedReader bufferedReader) throws IOException {
        String oneLine = " ";
        while ((oneLine = bufferedReader.readLine()) != null) {
            for (int i = latin_words_upper.length - 1; i >= 0; i--) {
                oneLine = oneLine.replaceAll(latin_words_upper[i], cyrill_words_upper[i]);
            }
            for (int length = latin_words_lower.length - 1; length >= 0; length--) {
                oneLine = oneLine.replaceAll(latin_words_lower[length], cyrill_words_lower[length]);
            }
            for (int i = latin.length - 1; i >= 0; i--) {
